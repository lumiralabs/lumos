import pytest
from lumos.book.toc_ai import detect_toc_pages, get_offset, extract_toc
import fitz
...
def test_get_offset(book_name, range, expected_offset):
    ...
    toc = extract_toc(pdf_path, range)
    offset = get_offset(toc, pages_str, start_offset=max(range))
...
#     with fitz.open(pdf_path) as doc:
#         total_pages = len(doc)
#     _toc = extract_toc_ai(pdf_path, page_ranges)
#     sections = _get_section_hierarchy(_toc, total_pages)
